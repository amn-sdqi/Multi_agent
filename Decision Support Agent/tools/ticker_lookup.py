...
#     """Searches Yahoo Finance for matching ticker symbol."""
...
#         if len(names) < 2:
#             return "❌ Please provide at least two company names separated by commas."
...
#             except Exception as e:
#                 return f"❌ Error finding ticker for '{name}': {str(e)}"
...
#                 logger.warning(f"⚠️ No ticker found for '{name}'. Skipping...")
#             except Exception as e:
...
class TickerLookupTool(BaseTool):
    ...
    def _run(self, query: str, run_manager=None) -> str:
        ...
        if invalid_names:
            return f"⚠️ Invalid company names: {', '.join(invalid_names)}. Please try again."
